...
try {
    ...
    if ($stmt === false) {
        throw new mysqli_sql_exception('Failed to prepare statement for like check.');
    }
    ...
    if ($res->num_rows > 0) {
        ...
        if ($del === false) {
            throw new mysqli_sql_exception('Failed to prepare statement for unliking.');
        }
        ...
        if ($ins === false) {
            throw new mysqli_sql_exception('Failed to prepare statement for liking.');
        }
    ...
    if ($likeCountRes === false) {
        throw new mysqli_sql_exception('Failed to get updated like count.');
    }
    ...
    error_log("Database error in like_post.php: " . $e->getMessage());
    if ($is_ajax) {
        header('Content-Type: application/json');
        echo json_encode(['success' => false, 'message' => 'A database error occurred. Please try again later.']);
        exit;
    } else {
        header("Location: view_post.php?id=$postId&error=db_error");
        exit;
    }
}
?>
